...
JVM은 자바 가상 머신의 약자로, 컴퓨터가 자바 바이트코드를 실행시키는 역할을 합니다. JVM은 OS의 종류에 구애받지 않고서도 자바 파일이 실행될 수 있도록 중개자 역할을 해줍니다. 
...
오버라이딩은 부모 클래스가 가지고 있는 메서드를 자식 클래스에서 재정의해서 사용하는 것을 말합니다. 오버라이딩은 코드를 재사용하기 위해서 사용하는데요. 자식 클래스가 부모 클래스를 상속 받아 중복 코드를 없애고, 자식 클래스마다 재정의가 필요한 부문만 오버라이딩하여 추가로 구현할 수 있습니다.

오버로딩은 메서드의 이름은 동일하나 매개변수의 개수 또는 타입의 종류가 다르면 같은 이름을 재정의해서 사용할 수 있는 것을 말합니다. 오버로딩의 이점은 같은 기능을 하는 메서드를 하나의 이름으로 사용할 수 있다는 것입니다.
...
- `접근 제어자` 클래스, 인터페이스, 멤버 변수/함수 등의 접근을 제어하는 지시어
...
- 변수나 메서드 또는 클래스가 '변경 불가능' 하도록 만듦
- `final`로 선언 시, 한 번 초기화 가능
    - final 필드가 초기화 되면, 그 초기값이 최종적인 값이 되어 프로그램 실행 도중 수정 불가
...
데이터 타입(data type, 자료형)을 일반화(generalize)하는 것으로, 클래스나 메소드에서 사용할 내부 데이터 타입을 클래스 외부에서 컴파일 시에 미리 지정하는 것입니다. 
<> 괄호 안에 타입을 지정해 줍니다. 
...
타입을 미리 명시해서 컴파일 시에 객체 타입이 체크 되므로 의도하지 않은 타입의 객체가 저장되는 것을 막을 수 있습니다. 또한 잘못된 형 변환을 막을 수 있기 때문에 안정성도 높아집니다.
...
일반 변수의 수명은 특정 코드 블록 범위 내에서만 유효하지만,
...
쓰레드 풀 환경에서 ThreadLocal을 사용하는 경우 ThreadLocal 변수에 보관된 데이터의 사용이 끝나면 반드시 해당 데이터를 삭제해 주어야 합니다. 

그렇지 않을 경우 재사용되는 쓰레드가 올바르지 않은 데이터를 참조할 수 있기 때문입니다.
