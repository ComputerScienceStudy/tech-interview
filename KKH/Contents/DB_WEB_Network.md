...
- **쿠키란?**
    ...
    쿠키는 클라이언트의 상태 정보를 로컬에 저장했다가 참조하는데 이때, 유효시간 이내라면 사용자 인증이 유지된다는 특징을 갖고 있습니다.
...
**🤔 쿠키와 세션이 필요한 이유에 대해 설명해주세요.**

HTTP 프로토콜 환경은 “connectionless, stateless”한 특성을 가지기 때문에 서버는 클라이언트가 누구인지 매번 확인해야합니다. 이 특성을 보안하기 위해 쿠키와 세션을 사용합니다.
...
- **stateless이란?**
    ...
    통신이 끝나면 서버가 클라이언트의 상태 정보를 유지하지 않는 특성입니다.
    

**🤔 쿠키와 세션의 동작 방식에 대해서 설명해주세요.**
...
- 쿠키
    - 방문 사이트 로그인 시
    - 쇼핑몰의 장바구니 기능
    - 자동 로그인, 팝업에 “오늘 더 이상 이 창을 보지 않음” 체크

- 세션
    - 로그인과 같이 보안상 중요한 작업을 수행할 때 사용됩니다.
    

**🤔 쿠키와 세션의 차이에 대해 알려주세요.**

- 쿠키는 사용자의 정보가 저장되는 위치가 클라이언트인 반면에 세션은 서버의 메모리입니다.
- 쿠키는 클라이언트 로컬에 저장되기 때문에, 변질되거나 요청시 스니핑 당할 우려가 있어 보안에 취약하지만 세션은 클라이언트에는 세션 아이디만 저장하고 있고 서버에서 처리하기 때문에 비교적 보안성이 좋습니다.
- 쿠키와 세션 모두 만료시간을 설정할 수 있지만, 쿠키의 경우 브라우저를 종료해도 정보가 남아있지만, 세션은 브라우저 종료 시 정보도 삭제됩니다.
- 쿠키는 정보가 쿠키에 있기 때문에 서버 요청시 빠른 반면에 세션은 서버에서 추가적인 처리가 요구되므로 비교적 느린 속도를 갖습니다.
...
### 세션 기반 인증 방식과 토큰 기반 인증 방식의 차이에 대해 설명해주세요.

---

**핵심답변**

세션 기반 인증 방식은 서버 측에서 사용자들의 정보를 기억하기 때문에 Stateful 서버라고 할 수 있습니다. 반면에 토큰 기반 인증 방식의 경우 인증 받은 사용자들에게 토큰을 발급하고, 서버에 요청을 할 때 헤더에 토큰을 함께 보내도록하여 유효성 검사를 합니다. 즉, 서버 기반의 인증 시스템과 달리 상태를 유지하지 않으므로 Stateless한 구조를 갖고 있습니다.

**🤔 동작 방식에 대해 설명해주세요.**

- **세션 기반 인증 방식**
    
    클라이언트로부터 요청 받을 경우 서버에서 세션을 생성 유지하고 세션 ID를 클라이언트에게 응답합니다. 이후 클라이언트에게 세션 ID와 함께 요청이 들어오면 서버에서는 세션 저장소에서 세션 ID를 기반으로 클라이언트의 정보를 찾아 응답해줌으로서 클라이언트의 상태를 유지시켜줍니다.
    
    ![image](https://user-images.githubusercontent.com/82690689/155175560-a461148c-0fbe-4b49-9e5b-0ab7f9fe709d.png)
...
- **토큰 기반 인증 방식**
    ...
    사용자가 로그인을 하면 서버 측에서는 해당 정보를 검증하고 정확하다면 토큰을 발급합니다.
    
    클라이언트는 토큰을 저장해두고, 서버에 요청을 할 때마다 토큰을 서버에 함께 전달합니다. 이때, HTTP 요청 헤더에 토큰을 포함시킵니다.
    
    서버는 토큰을 검증하고, 요청에 응답합니다.
    
    ![image](https://user-images.githubusercontent.com/82690689/155175596-dfc10230-2bc5-425e-a48a-21e0ee7a5358.png)
...
- **세션기반인증방식**
    - 서버에서 클라이언트의 상태를 유지하고 있어 로그인 여부를 확인할 수 있고 경우에 따라서 강제 로그아웃 등의 제재를 가할 수 있습니다.
    - 사용자 인증 정보를 서버에 저장해야하기 때문에, 메모리에 부하가 걸리게 됩니다.
    - 스케일 아웃이 발생할 경우 세션을 분산시키기 위한 시스템을 설계해야 합니다.
- **토큰기반인증방식**
    - 토큰이 클라이언트 측에 저장되므로 서버는 Stateless하며 스케일 아웃이 발생해도 헤더에 토큰만 담겨있다면 어떤 요청이 와도 상관 없습니다.
    - 토큰에 선택적인 권한만 부여하여 발급할 수 있습니다.(OAuth)
    - CORS 문제를 해결할 수 있습니다.
...
토큰 자체를 정보로 사용하는 Self-Contained 방식으로 정보를 안전하게 전달할 수 있스

**🤔 JWT 구조에 대해서 설명해주세요.**

JWT는 Header, Payload, Signature의 3 부분으로 이뤄지며, Json 형태인 각 부분은 Base64로 인코딩 되어 표현됩니다. 또한 각 부분을 이어 주기 위해 . 구분자를 사용합니다.

- Header(헤더) : Signature를 해싱하기 위한 알고리즘과 알고리즘을 저장하는 방식이 포함됩니다.
- Payload(페이로드) : 토큰에서 사용할 정보의 조각들인 클레임이 담겨있습니다.
- Signature(서명) : 토큰을 인코딩하거나 유효성 검증을 할 때 사용하는 고유한 암호화 코드입니다.

**🤔 JWT를 사용하는 이유가 무엇인가요?**

쿠키인증방식과 세션인증방식은 각각의 문제를 갖고있습니다. 쿠키인증방식의 경우 쿠키를 보내기 때문에 스니핑에 취약하고 세션인증방식의 경우 서버 메모리에 발생하는 부하와 스케일 아웃과 같은 부분에 문제를 갖고 있습니다. 반면에 JWT는 두 인증방식이 갖는 문제점을 해소해 줄 수 있기 때문입니다.

**🤔 JWT의 장,단점에 대해 알려주세요.**

- JWT 자체가 사용자 인증에 필요한 모든 정보를 포함하고 있기 때문에 별도의 인증 저장소가 필요 없습니다.(Self-Contained)
- 쿠키를 전달하지 않아도 되므로 쿠키를 사용함으로써 발생하는 취약점이 사라집니다.
- 토큰 자체에 정보를 담고 있으므로 양날의 검이 될 수 있습니다.(Self-Contained)
- 토큰에 담는 정보가 많아질수록 토큰 길이가 늘어나 네트워크에 부하를 줄 수 있습니다.
- 만료 시간을 명시해주지 않을 경우 삭제가 되지 않으므로 보안에 취약해질 수 있습니다.
...
웹 서버는 HTTP 프로토콜을 기반으로 하여 클라이언트의 요청을 서비스하는 기능을 담당한다. 이때, 담당하는 기능은 정적인 컨텐츠 제공과 클라이언트의 요청을 WAS로 보내고 응답을 클라이언트에 전달하는 기능을 갖고 있습니다.

반면에, WAS는 웹서버와 웹 컨테이너가 합쳐진 것으로 DB를 조회하거나 다양한 로직처리를 요구하는 동적인 컨텐츠를 제공합니다.

**🤔 Web Container란?**

서블릿 컨테이너라고도 하며, JSP, Servlet을 실행시킬 수 있는 소프트웨어를 의미합니다.

---

<br><br>   

### 웹 공격 패턴과 방어 방법에 대해 설명해주세요.

---

**🤔 SQL Injection에 대해서 간단하게 설명해주세요.**

임의의 SQL문을 주입하고 실행되게하여 데이터베이스가 비정상적인 동작을 하도록 조작하는 행위입니다.

화이트리스트 체크를 통해 방어할 수 있습니다.
...
![image](https://user-images.githubusercontent.com/82690689/155175666-6c4a55e0-398e-4cb8-b98d-58b35074f3a1.png)
...
### RESTful의 개념에 대해서 설명해주세요.

---

**핵심 답변**

RESTful은 REST라는 아키텍처를 구현하는 웹 서비스를 말합니다.

즉, REST 아키텍처를 기반으로 이해하기 쉽고 사용하기 쉬운 REST API를 만드는 것을 RESTful하다고 할 수 있습니다.

**🤔 RESTful이란 무엇이며, 이것에 대해 아는대로 설명해주세요.**

- URI 마지막 문자로 슬래시(/)를 포함하지 않는다.
- URI에는 밑줄(_)보다 하이픈(-)을 사용한다.
- URI 경로에는 소문자를 사용한다.
- URI에는 자원(resource)만을 이용해 작성한다.
...
### 웹 브라우저에서 URI에 구글 닷컴을 쳤을 때 발생하는 일들을 아는 대로 설명해주세요.(DNS 서버, HTTP 프로토콜, HTTPS 프로토콜, DOM, IP, PORT 등등의 용어를 사용해서)
...
### 사용자가 웹브라우저를 통해 서버에 이미지를 요청해서 사용자에게 보여주기까지 과정을 설명해주세요.

---

**핵심답변**

이미지 파일과 같은 정적인 파일들은 웹 문서(HTML)가 클라이언트로 보내질 때 함께 가지 않습니다.

클라이언트는 HTML 문서를 먼저 받고 그에 맞게 필요한 이미지 파일들을 다시 서버로 요청하면 그때서야 이미지 파일들을 받습니다.
...
**🤔 OSI 7계층이 무엇인지 그 존재 이유에 대해서 설명해주세요.**

OSI 7계층이란 네트워크에서 통신이 일어나는 과정을 7단계롤 나눈 것을 말합니다.

통신이 일어나는 과정을 단계별로 파악할 수 있고, 7단계 중 특정한 곳에 이상이 생기면 해당 단계만 고쳐줌으로서 문제를 해결할 수 있게됩니다.

- 1계층 - 물리계층
    
    이 계층에서는 전기적, 기계적, 기능적인 특성을 이용해서 통신 케이블로 데이터를 전송하게 됩니다. 통신 단위는 비트입니다.
    
    케이블, 리피터, 허브를 통해 데이터를 전송합니다.
    
- 2계층 - 데이터 링크계층
    
    브릿지나 스위치를 통해 맥 주소를 가지고 물리계층에서 받은 정보를 전달합니다.
    
    프레임에 주소를 부여합니다.(MAC - 물리적 주소)
    
- 3계층 - 네트워크 계층
    
    데이터를 목적지까지 안전하고 빠르게 전달하는 기능을 갖습니다.(라우팅)
    
    주소부여(IP), 경로 설정(Route)
    
- 4계층 - 전송 계층
    
    TCP, UDP 프로토콜을 이용해서 패킷을 생성하고 전송합니다.
    
- 5계층 - 세션 계층
    
    데이터가 통신하기 위한 논리적인 연결을 합니다.
    
    통신을 하기 위한 세션을 확립/유지/중단을 합니다.
    
- 6계층 - 표현 계층
    
    사용자의 명령어를 완성 및 결과를 표현합니다.
    
    예) 해당 데이터가 TEXT인지, GIF인지 등
    
- 7계층 - 응용 계층
    
    응용 프로세스와 직접 관계하여 일반적인 응용 서비스를 수행합니다.
    
    네트워크 소프트웨어 UI 부분, 사용자의 입출력 부분입니다.
    
![image](https://user-images.githubusercontent.com/82690689/155175731-9fe2d6a4-850a-49ca-89fd-98096f279794.png)

[https://shlee0882.tistory.com/110](https://shlee0882.tistory.com/110)

- 캡슐화 / 역캡슐화
    
    송신 측 관점에서 상위 계층에서 하위 계층으로 데이터를 전달할 때, 각 계층의 프로토콜 제어 정보를 헤더로 붙여 전달합니다. 이런 과정을 캡슐화라고 합니다.
    
    반대로, 수신 측 관점에서 캡슐화된 데이터가  하위 계층에서 상위 계층으로 데이터를 전달하는 것을 역 캡슐화라고 합니다.
    

**🤔 TCP/IP 4계층에 대해 설명해주세요.**

인터넷 통신을 하는데 있어 가장 기반이 되는 프로토콜로 TCP와 IP를 기반으로 통신합니다.

- 1계층 - 네트워크 액세스 계층
    
    물리적인 주소로 MAC을 부여하고 LAN을 이용해 통신합니다.
    
- 2계층 - 인터넷 계층
    
    통신 노드 간의 IP 패킷을 전송하는 기능과 라우팅 기능을 담당합니다.
...
- 4계층 - 응용 계층
    ...
    FTP,HTTP,SSH 프로토콜이 있습니다.
    

**🤔 웹 서버 소프트웨어(Apache, Nginx)는 OSI 7계층 중 어디서 작동하는지 설명해주세요.**

**🤔 웹 서버 소프트웨어(Apache, Nginx)의 서버 간 라우팅 기능은 OSI 7계층 중 어디서 작동하는지 설명해주세요.**
...
도메인 네임 시스템으로 웹사이트의 IP주소와 도메인 주소를 이어주는 환경/시스템을 의미합니다.

즉, 웹사이트의 IP 주소를 특정 도메인 주소와 같다는 기록을 저장해두고, 인터넷 사용자들이 도메인 주소를 검색했을 때 IP 주소로 연결되도록 도와주는 역할을 합니다.
...
**🤔 HTTP가 무엇인지 그 역할은 어떤게 있는지 알려주세요.**

HTTP란, 텍스트 기반의 통신 규약으로 인터넷에서 데이터를 주고받을 수 있는 프로토콜입니다. 규약을 정해두었기 때문에 모든 프로그램을 이 규약에 맞춰 개발해서 서로 정보를 교환할 수 있게되었습니다.

**🤔 HTTP와 HTTPS의 차이를 설명해주세요.**

HTTPS는 HTTP에 데이터 암호화가 추가된 프로토콜입니다. 80번 포트를 사용하는 HTTP와는 다르게 443번 포트를 사용하며, 네트워크 상에서 중간에 제 3자가 정보를 볼 수 없도록 암호화를 지원합니다.

**🤔 HTTPS 동작 과정**

1. 브라우저가 서버로 최초 연결을 시도합니다.
2. 서버는 공개키(인증서)를 브라우저에게 보냅니다.
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/c19c1313-0973-4692-a200-33750e8dce5e/Untitled.png)

**🤔 HTTPS에 대해서 설명하고 SSL Handshake에 대해서 설명해주세요.**

HTTPS 연결 과정으로 먼저 서버와 클라이언트 간에 세션키를 교환합니다. 이때, 세션키는 주고 받는 데이터를 암호화하기 위해 사용되는 대칭키입니다.

이때, 이 세션키를 클라이언트와 서버간에 교환할 때, 비대칭키가 사용됩니다.

즉, 처음 연결을 성립하여 안전하게 세션키를 공유하는 과정에서 비대칭키가 사용되는것입니다. 이후에는 데이터를 교환하는 과정에서 빠른 연산 속도를 위해 대칭키가 사용됩니다.

**🤔 HTTP 프로토콜의 특징**

- HTTP 메시지는 HTTP 서버와 HTTP 클라이언트에 의해 해석이 됩니다.
- TCP/IP를 이용하는 응용 프로토콜입니다.
- HTTP는 연결상태를 유지하지 않는 비연결성 프로토콜입니다.
- HTTP는 연결을 유지하지 않는 프로토콜이기 때문에 요청/응답 방식으로 동작합니다.

**🤔 HTTP 1.1 VS 2.0 VS 3.0**

**🤔 HTTP Method - PUT과 PATCH의 차이에 대해서 알려주세요.**

PUT은 리소스의 모든 것을 업데이트 하는 반면에, PATCH는 리소스의 일부만 업데이트합니다.

예를 들어 이름,나이,성별 컬럼을 갖는 테이블이 있을 때, 홍길동, 20, 남자라는 값을 갖는 데이터가 있다면, PUT 메서드로 이름만 김광훈으로 요청을 보낸다면 나이와 성별은 null값이 되버린다.

반면에, PATCH 메서드로 이름만 김광훈으로 요청을 보낸다면 이름만 변경되고 나이와 성별으 그대로 20, 남자를 유지하게 됩니다.
...
TCP와 UDP는 전송 정보를 담기위한 프로토콜로 공통적으로 IP와 PORT 그리고 체크섬 정도의 전송 정보를 담는 패킷을 만듭니다.

이때, TCP는 추가적으로 3 way handshake, 데이터 전달 보증, 순서 보증 등의 추가 정보를 함께 패킷을 만듭니다.

즉, TCP가 UDP보다 데이터 전송에 있어 신뢰성 있다고 볼 수 있습니다.

**🤔 3 way hand shake에 대해서 설명해주세요.**

장치들 사이에 논리적인 접속을 성립시키기 위해 사용합니다.

즉, 3 way hand shake를 통해 양쪽 모두 데이터를 전송할 준비가 되었다는 것을 보장받을 수 있습니다.

- 동작과정
    1. 클라이언트가 서버에 접속을 요청하는 SYN 패킷을 보냅니다. 이때, 클라이언트는 SYN을 보내고 SYN/ACK 응답을 기다리는 SYN_SENT 상태가 됩니다.
    2. 서버는 SYN 요청을 받고 클라이언트에게 요청을 수락합니다. ACK와 SYN flag가 설정된 패킷을 발송하고 클라이언트가 다시 ACK로 응답하기를 기다립니다. 이때, 서버는 SYN_RECEIVED 상태가 됩니다.
    3. 클라이언트는 서버에게 ACK을 보내고 이후에는 연결이 이뤄지고 데이터가 오갑니다. 이때 서버 상태는 ESTABLISHED입니다.
    

**🤔 4 way hand shaking에 대해서 설명해주세요.**

세션을 종료하기 위해 수행되는 절차입니다.

1. 클라이언트가 연결을 종료하겠다고 FIN 플래그를 전송합니다.
2. 서버가 확인메시지를 보내고 자신의 통신이 끝날때까지 기다리는데 이 상태를 CLOSE_WAIT라고합니다.
